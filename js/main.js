 ...
 *  - Print: @page rule injection (theme-dependent) for Ctrl+P / right-click → Print
 *  - Download: header button fetches the pre-built PDF (see .github/workflows)
 *  - i18n: apply translations, lang toggle, localStorage persistence, ?lang= override
 *
...
/**
 * ?lang=pt|en overrides localStorage for this load only — never persisted.
 * This is how the PDF export workflow renders a fixed language without
 * seeding a browser profile; see also the ?theme= override in <head>.
 */
let currentLang = getUrlParam('lang') === 'en' ? 'en'
  : getUrlParam('lang') === 'pt' ? 'pt'
  : (readStored(LS_KEY) || DEFAULT_LANG);
...
/** URLSearchParams can be unavailable in exotic environments; fail quiet. */
...
/**
 * The header button no longer calls window.print(). Instead it downloads the
 * pre-built PDF that the "Generate CV exports" GitHub Action produces on
 * every push to main (exports/cv-pt.pdf / exports/cv-en.pdf) — that file is
 * guaranteed selectable/clickable because it was generated by Puppeteer
 * server-side, never routed through a system print driver.
 *
 * Ctrl+P / right-click → Print still go through the browser's native dialog
 * and still render via print.css with the light/dark @page rule from
 * applyPageRule() — that path is untouched by this change.
 */
function setupDownloadButton() {
  ...
  btn.addEventListener('click', function () {
    ...
    a.href = 'exports/cv-' + lang + '.pdf';
    a.download = 'bruno-silvestre-cv-' + lang + '.pdf';
